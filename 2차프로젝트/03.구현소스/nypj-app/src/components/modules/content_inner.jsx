...
export function Inner({ type, data, idx }) {
   ...
   // 위시리스트 추가함수
   let addWishFn = (e) => { 
    // 130줄에서는 잘찍히는 상품데이터가 여기서는 잘 찍히지 않음
 console.log("씨불", itemdata)
 console.log('idx',idx)
   };
  ...
  //  // 위시리스트 추가함수
  //  let addWishFn = (e) => {
  //     let wishList = myCon.wishList;
  //     // 추가할 아이템  ---- 이부분에서 자꾸 예전값이 찍힘 
  //     let itemdata = allProducts.find((v) => v.idx === idx);
  ...
  //        // 저장데이터의 idx 해당 idx가 일치하는게 있는지 확인
  //        let isinWish = wishList.some((v) => v.idx === idx);
  //        console.log(" >>>", wishList, idx);
  //        console.log("현재 wishList 갯수", wishList.length);
  //        console.log("위시리스트 포함여부:", isinWish);

  //        if (isinWish) {
  //           // 이미 해당상품이 위시리스트에 있으면
  //           alert("이미 장바구니에 담긴 아이템 입니다.");
  //        } 
  //       else if(!isinWish) {
  //           // 위시에 없으면 기존배열에 직전값에 추가하기
  //           result = [...wishList, itemdata];
  //           myCon.setWishList(result);
           
  //           // 아래코드에 바로 반영안됨-->  myCon.setWishList((prev) => [...prev, itemdata]);
  //           wishList = result
  //     } 
  //   }
  //     // 데이터가 존재하지 않을때 (여기는 문제 없음 )
  //     else {
  //        result = wishList.push(itemdata);
  //        myCon.setWishList(result);
  //     }

  //     console.log("결과 wishList :", wishList);
  //     // 결과 제이슨 문자화
  //     wishList = JSON.stringify(wishList);
  //     // 결과 로컬스 실제 반영
  //     localStorage.setItem("wish-data", wishList);
  //  };


   useEffect(() => {
      // 위시리스트 버튼 이벤트 함수
      $(".wish-btn").on("click", (e) => addWishFn());

      return () => {
         // 소멸시 이벤트 지우기
         console.log("위시 이벤트 지워!");
         $(".wish-btn").off("click", (e) => addWishFn());
      };
   });
