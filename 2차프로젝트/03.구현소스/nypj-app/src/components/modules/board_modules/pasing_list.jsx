...
export const PagingList = ({
  ...
  //   // 새 페이징의 페이징번호
  //   let newcurrentPageBlockNum;

  //   // 1. opt 옵션에 따라 페이징의 페이징이동번호 만들기
  //   // (1) 일반 페이징이동은 현재페이징번호에 증감
  //   if (opt) newcurrentPageBlockNum = currentPageBlockNum.current + dir;
  //   // (2) 끝 페이징이동은
  //   // 오른쪽(1)일 경우 맨끝 페이징번호로 이동(totalPageBlockCount)
  //   // 왼쪽(-1)일 경우 맨앞 페이징번호로 이동(1)
  //   else newcurrentPageBlockNum = dir === 1 ? totalPageBlockCount : 1;

  //   // 2.페이징의 페이징 번호 업데이트하기
  //   currentPageBlockNum.current = newcurrentPageBlockNum;
